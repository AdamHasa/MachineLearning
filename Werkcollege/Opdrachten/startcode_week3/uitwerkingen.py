...
def build_model():
    ...
    # Voeg de inputlaag toe
    model.add(tf.keras.layers.Flatten(input_shape=(28, 28)))  # 28x28 input nodes

    # Voeg de verborgen laag toe met 128 nodes en ReLU-activatie
    model.add(tf.keras.layers.Dense(128, activation='relu'))

    # Voeg de outputlaag toe met 10 nodes (voor de 10 klassen) en softmax-activatie
    model.add(tf.keras.layers.Dense(10, activation='softmax'))

    # Compileer het model met de opgegeven parameters
...
def conf_els(conf, labels): 
    ...
    for i in range(len(labels)):
        ...
        metrics.append((str(category), tp, fp, fn, tn))
...
def conf_data(metrics):
    ...
    rv = {'tpr':trp, 'ppv':ppv, 'tnr':tnr, 'fpr':fpr }
    return rv
